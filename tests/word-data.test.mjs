...
test("the first four collectible artworks are bundled", async () => {
  for (const file of ["nova-nibbler.jpg", "stelluna.jpg", "bubblebloom.jpg", "sunfeather.jpg"]) {
    assert.match(page, new RegExp(`creatures/${file}`));
    await access(new URL(`../public/creatures/${file}`, import.meta.url));
  }
});
